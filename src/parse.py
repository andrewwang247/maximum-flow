...
def create_network(filepath: Path) -> FlowNetwork:
    ...
    with filepath.open(encoding="UTF-8") as fin:
        ...
        assert first_tokens[0] == "p", 'Problem statements begin with "p"'
        assert first_tokens[1] == "max", "Only max flow problems are allowed."
        nodes = int(first_tokens[2])
        arcs = int(first_tokens[3])

        second_tokens = _get_next_line(fin)
        assert second_tokens[0] == "n", 'Source statements begin with "n"'
        assert len(second_tokens) == 3, "Source statements require 2 values."
        assert second_tokens[2] == "s", 'Source statements end with "s"'
        source = int(second_tokens[1])

        third_tokens = _get_next_line(fin)
        assert third_tokens[0] == "n", 'Sink statements begin with "n"'
        assert len(third_tokens) == 3, "Sink statements require 2 values."
        assert third_tokens[2] == "t", 'Sink statements end with "t"'
